...
intitiaze_monitor_fd_set(){
    ...
    for(; i < MAX_CLIENT_SUPPORTED; i++)
        monitored_fd_set[i] = -1;//initilises to all arry with -1 represnts emptty
}
...
refresh_fd_set(fd_set *fd_set_ptr){
    ...
    FD_ZERO(fd_set_ptr);//removes all the holded FDs in fd_set datastructure
    int i = 0;
    for(; i < MAX_CLIENT_SUPPORTED; i++){
        if(monitored_fd_set[i] != -1){     //recopies all the fds present in our array to fd_set 
